...
type PagesProvider(config: TypeProviderConfig) as this =
    inherit
        TypeProviderForNamespaces(
            ...
            assemblyReplacementMap = [
                ("EvgTsvDotMe.TypeProviders.DesignTime", "EvgTsvDotMe.TypeProviders.Runtime")
            ],
    ...
    let rec createPageTy (info: PageInfo) =
        ...
        | PageInfo(name, path) ->
            let pageTy =
                ProvidedTypeDefinition(asm, ns, name, Some typeof<obj>, isErased = true)
    ...
    do
        let pagesContainer =
            ProvidedTypeDefinition(asm, ns, "PagesProvider", Some typeof<obj>, isErased = true)

        let staticParams = [
            ProvidedStaticParameter("PagesDir", typeof<string>)
            ProvidedStaticParameter("BasePath", typeof<string>, parameterDefaultValue = "/pages")
        ]
